...
<head>
  ...
  <style>
    :root{
      ...
      /* === Global scale (ย่อ/ขยายทั้งระบบ) === */
      --scale:.88; /* ปรับช่วง .80–1.00 ได้ตามชอบ */
  
      /* === Fluid type scale (ย่อ-ขยายตามจอ) === */
      --fluid-base: calc( clamp(13px, 0.9vw + 9px, 16px) * var(--scale) ); /* ฐานทั้งหน้า */
    ...
    *{box-sizing:border-box}
    ...
    body{
      ...
      line-height:1.55; font-size: 1rem; /* ใช้ rem เพื่อให้สเกลตาม html */
    }
    ...
    h4{ font-size: calc( clamp(16px, 1.2vw + 11px, 20px) * var(--scale) ); }
    h5{ font-size: calc( clamp(15px, 1.0vw + 10px, 18px) * var(--scale) ); }
    h6{ font-size: calc( clamp(14px, 0.8vw +  9px, 16px) * var(--scale) ); }
    ...
    .brand h1{font-size:1rem; margin:0; letter-spacing:.2px} /* fluid ผ่าน rem */
    ...
    .td-long{
      word-break: normal;
      overflow-wrap: break-word;  /* ตัดเฉพาะคำยาว/ไม่มีช่องว่างจริง ๆ */
      hyphens: auto;
    }
    ...
    [style*="font-size:17px"]{ font-size: var(--fz-17) !important; }
    ...
    /* ยูทิลิตี้ */
    .btn{ font-size: var(--fz-13); }
    .btn-cell{ font-size: var(--fz-12); }
    .pill{ font-size: var(--fz-12); }
    .chip{ font-size: var(--fz-12); }
    .status-select{ font-size: var(--fz-12); }
    .modal-title{ font-size: var(--fz-18) !important; }
    ...
    @media (max-width: 420px){
      :root{
        --fluid-base: calc( clamp(12px, 1.8vw + 8px, 15px) * var(--scale) ); /* กันตกรูปแบบบนจอแคบมาก */
...
<div class="view" id="view-products" aria-hidden="false">
      ...
      <form method="GET" style="padding:16px 16px 18px;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px">
        ...
        @if($has('PriceTHB'))
          <div>
            ...
            <input name="PriceTHB" inputmode="decimal" placeholder="เช่น 399248.8" style="width:100%;height:40px;border:1px solid #e5e7eb;border-radius:10px;padding:0 10px">
          </div>
        @endif
  ...
  <script>
    ...
    // ===== Highlight คำค้นใน Users (ไม่ทำลาย <wbr>) =====
    (function(){
    ...
    // ===== ใส่ <wbr> อัตโนมัติ (เฉพาะข้อความ > 20 ตัว) =====
    (function(){
      ...
      document.querySelectorAll('[data-wbr]').forEach(el => {
        ...
        if (raw.length <= 20) return; // สั้นกว่า/เท่ากับ 20 ไม่ต้องแทรก
        const N = parseInt(el.getAttribute('data-wbr'), 10) || 15;
        insertWbr(el, N);
      });
    })();

    // ===== บังคับ 1 บรรทัดถ้าเนื้อหาไม่เกิน 20 ตัวอักษร (ทุกตาราง) =====
    (function(){
      const MAX = 20;
      const tds = document.querySelectorAll('table tbody td');
      tds.forEach(td=>{
        if (td.querySelector('button, input, select, textarea, .pw')) return; // ข้าม cell ที่เป็นคอนโทรล
        const text = (td.textContent || '').replace(/\s+/g,'');
        if (text.length > 0 && text.length <= MAX) td.classList.add('nowrap20');
      });
    })();

    // ===== Orders: ค้นหาแบบ client-side =====
    (function(){
      const qEl = $('#orderQ');
