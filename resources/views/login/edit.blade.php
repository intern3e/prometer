  ...
  <main class="max-w-4xl mx-auto px-4 md:px-6 py-10">
    ...
    <div class="card soft p-6">
      <form id="contactForm" class="space-y-6" novalidate>
        @csrf
        <input type="hidden" name="active_tab" id="active_tab" value="{{ old('active_tab','contact') }}">
        <input type="hidden" id="shipping_address" name="shipping_address" value="{{ old('shipping_address') }}">
          ...
          <div>
            ...
            <div class="relative mt-1">
              ...
              <select id="customer_type" name="customer_type"
                ...
                @endphp
                <option value="person"  {{ $typecust === 'person'  ? 'selected' : '' }} data-i18n="type_person">บุคคล</option>
                <option value="company" {{ $typecust === 'company' ? 'selected' : '' }} data-i18n="type_company">บริษัท/หน่วยงาน</option>
              </select>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="relative mt-1">
              ...
              <input type="text" id="main_namecontact" name="main_namecontact" required
                     ...
                     value="{{ old('main_namecontact', $custdetail->main_namecontact ?? '') }}">
            </div>
          </div>
            ...
            <div>
              ...
              <div class="relative mt-1">
                ...
                <input type="email" id="email_contact" name="email_contact" autocomplete="email"
                       ...
                       value="{{ old('email_contact', $custdetail->email_contact ?? $custdetail->email ?? '') }}">
              </div>
            </div>
            <div>
              ...
              <div class="relative mt-1">
                ...
                <input type="tel" id="tel_contact" name="tel_contact" autocomplete="tel" inputmode="tel"
                       ...
                       value="{{ old('tel_contact', $custdetail->tel_contact ?? '') }}">
              </div>
            </div>
          </div>

          @php $isCompany = ($typecust === 'company'); @endphp
          <div id="companyWrap" class="{{ $isCompany ? '' : 'hidden' }}">
            <label class="block text-sm font-medium text-slate-700" data-i18n="company_name_hint">ชื่อบริษัท (ถ้ามี)</label>
            <div class="relative mt-1">
              <i class="bi bi-building input-icon"></i>
              <input type="text" id="company_name" name="company_name" {{ $isCompany ? 'required' : '' }}
                     class="w-full rounded-xl border border-slate-300 pl-10 pr-3 py-2 ring-brand focus:border-amber-400"
                     data-i18n-attr="placeholder" placeholder="เช่น Prometer Co., Ltd" autocomplete="organization"
                     value="{{ old('company_name', $custdetail->company_name ?? '') }}">
            </div>
        ...
        <section id="tabAddress" class="space-y-6 hidden" role="tabpanel" aria-labelledby="tabBtnAddress" tabindex="-1">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div class="relative">
              ...
              <input id="main_address" name="main_address" type="text" data-addr required
                     ...
                     value="{{ old('main_address', $custdetail->main_address ?? '') }}">
            </div>
            <div class="relative">
              ...
              <input id="main_subdistrict" name="main_subdistrict" type="text" data-addr
                     ...
                     value="{{ old('main_subdistrict', $custdetail->main_subdistrict ?? '') }}">
            </div>
            <div class="relative">
              ...
              <input id="main_district" name="main_district" type="text" data-addr required
                     ...
                     value="{{ old('main_district', $custdetail->main_district ?? '') }}">
            </div>
            <div class="relative">
              ...
              <input id="main_province" name="main_province" type="text" data-addr required
                     ...
                     value="{{ old('main_province', $custdetail->main_province ?? '') }}">
            </div>
            <div class="relative">
              ...
              <input id="main_postal" name="main_postal" type="text" inputmode="numeric" pattern="\d{5}" data-addr required
                     ...
                     value="{{ old('main_postal', $custdetail->main_postal ?? '') }}">
            </div>
            <div class="relative">
              ...
              <input id="main_country" name="main_country" type="text" data-addr required
                     ...
                     value="{{ old('main_country', $custdetail->main_country ?? 'ไทย') }}">
            </div>
          </div>
          ...
          <div>
            <div class="flex items-center justify-between mb-2">
              ...
              <span class="chip"><span class="tabular-nums">{{ count($subaddresses ?? []) }}</span> <span data-i18n="items_suffix">รายการ</span></span>
            </div>
  ...
  (function () {
    ...
    function createSubAddrBlock(){
      ...
      node.querySelector('.btn-save-subaddr').addEventListener('click',async()=>{
        ...
        try{
          const res=await fetch('{{ route("subaddress.address") }}',{method:'POST',headers:{'X-CSRF-TOKEN':csrf,'Accept':'application/json','Content-Type':'application/json'},body:JSON.stringify(payload)});
  ...
  (function () {
    ...
    form.addEventListener('submit',async(e)=>{
      ...
      try{
        ...
        const res=await fetch('{{ route("updateprofile.post") }}',{method:'POST',headers:{'X-CSRF-TOKEN':csrf,'X-Requested-With':'XMLHttpRequest','Accept':'application/json'},body:formData});
        ...
        if(data.success){ alert(data.success); window.location.assign('{{ route("profile.show") }}'); }
