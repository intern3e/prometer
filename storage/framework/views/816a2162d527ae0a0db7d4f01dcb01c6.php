...
<body>
  ...
  <main class="container-outer section-pad py-6">
    ...
    <nav class="breadcrumbs" aria-label="breadcrumb">
      <a href="<?php echo e(url('/')); ?>" data-i18n="bc_home">Home</a>
      <span aria-hidden="true">/</span>
      <span id="bcName" aria-current="page"><?php echo e($product->name); ?></span>
    </nav>
    ...
    <div id="productWrap" class="grid grid-cols-1 md:grid-cols-12 gap-6 mt-4">
      ...
      <section class="md:col-span-5">
        <div class="card p-3 soft">
          <div id="imgBox" class="aspect-square bg-white rounded-xl image-zoom flex items-center justify-center overflow-hidden" aria-label="product image">
            <img src="<?php echo e($imageUrl); ?>" alt="<?php echo e($product->name); ?>" class="w-full h-full object-contain" loading="eager" decoding="async">
          </div>
      ...
      <aside class="md:col-span-7">
        <div class="card p-5 soft">
          <input type="hidden" id="iditem" value="<?php echo e($product->iditem); ?>">

          <!-- Title + Model -->
          <?php $model = trim((string)($product->model ?? '')); ?>
          <h1 id="pName" class="text-xl md:text-2xl font-extrabold mb-1"><?php echo e($product->name); ?></h1>
          <?php if($model !== ''): ?>
            <p id="pModel" class="text-sm text-gray-400 mb-3">
              <span class="font-medium" data-i18n="label_model">Model:</span>
              <span><?php echo e($model); ?></span>
            </p>
          <?php endif; ?>
          ...
          <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
            <div>
              ...
              <div class="flex items-baseline gap-2 mb-2">
                <span id="pPrice" class="text-2xl md:text-3xl font-extrabold" style="color:var(--brand);" data-thb="<?php echo e(number_format($web ?? 0, 2, '.', '')); ?>">
                  <?php if(($web ?? 0) > 0): ?> <?php echo e('฿'.number_format($web, 2)); ?> <?php else: ?> <span data-i18n="label_quote">ขอใบเสนอราคา</span> <?php endif; ?>
                </span>
                <?php if(($base ?? 0) > ($web ?? 0) && ($web ?? 0) > 0): ?>
                  <span id="pBase" class="line-through text-sm text-gray-400" data-thb="<?php echo e(number_format($base ?? 0, 2, '.', '')); ?>"><?php echo e('฿'.number_format($base, 2)); ?></span>
                <?php endif; ?>
              </div>
              ...
              <?php if($qty !== null && $qty > 0): ?>
                <div class="mt-1 text-sm" style="color:var(--ok)">
                  ...
                  <span class="font-medium" style="color:#15803d">(<?php echo e($qty); ?> <span data-i18n="unit_piece">ชิ้น</span>)</span>
                </div>
              <?php else: ?>
              ...
              <div class="meta-body">
                <div class="meta-title">
                  ...
                  <span id="leadtimeText"
                        data-leadtime-raw="<?php echo e(trim($product->leadtime ?? '')); ?>"
                        data-source="<?php echo e(trim($product->source ?? '')); ?>"></span>
                </div>
...
<section id="pColJWrap" class="mb-4">
  <div class="border rounded-2xl overflow-hidden bg-white shadow-sm">
    ...
    <div id="pColJ" class="sr-only"><?php echo e(e($product->document)); ?></div>
  </div>
</section>
...
<?php endif; ?>
    ...
    <div id="notFound" class="hidden text-center py-16">
      ...
      <a href="<?php echo e(url('/')); ?>" class="mt-4 inline-block" style="color:var(--brand);" data-i18n="notfound_back">กลับหน้าแรก</a>
    </div>
  </main>
  ...
  <div id="stickyBar" class="sticky-cta" role="region" aria-label="sticky purchase bar">
    <div class="container-outer section-pad py-3 flex items-center gap-3">
      <div class="min-w-0 flex-1">
        <div id="stickyName" class="truncate font-semibold"><?php echo e($product->name ?? 'Loading...'); ?></div>
        <div id="stickyPrice" class="font-bold" style="color:var(--brand);" data-thb="<?php echo e(number_format($webSticky ?? 0, 2, '.', '')); ?>">
          <?php if(($webSticky ?? 0) > 0): ?> ฿<?php echo e(number_format($webSticky, 2)); ?> <?php else: ?> <span data-i18n="label_quote">ขอใบเสนอราคา</span> <?php endif; ?>
        </div>
      </div>
...
(function(){
  ...
  (function(){
    ...
    async function addToCart(payload){ const res = await fetch('<?php echo e(route("cart.add")); ?>', { method:'POST', credentials:'same-origin', headers:{ 'X-CSRF-TOKEN':csrf, 'Accept':'application/json', 'Content-Type':'application/json' }, body: JSON.stringify(payload) });
  ...
  <script>
    (function(){
      const RAW_STOCK = "<?php echo e(trim((string)($product->Stock ?? ''))); ?>";
      let qty = null;
...
<?php /**PATH C:\xampp\htdocs\tool\resources\views/test/product.blade.php ENDPATH**/ ?>