...
Route::get('/showproduct', function () {
    return redirect('/products', 301)->header('X-Robots-Tag', 'noindex, nofollow');
});
...
// ==================== robots.txt (เสริมให้ dev ใช้ได้ แม้ไม่มีไฟล์จริง) ====================
Route::get('/robots.txt', function () {
    $txt = "User-agent: *\nDisallow:\n\nSitemap: " . url('/sitemap.xml') . "\n";
    return response($txt, 200)->header('Content-Type', 'text/plain');
});

// ==================== sitemap.xml (ชั่วคราวให้มีเฉพาะหน้าแรก) ====================
Route::get('/sitemap.xml', function () {
    $home = rtrim(url('/'), '/') . '/'; // ให้ตรงกับ canonical

    $urls = [
        ['loc' => $home, 'changefreq' => 'weekly', 'priority' => '1.0'],
        // ช่วงนี้ “ยังไม่” ใส่ /products หรือสินค้าเดี่ยวลง sitemap
    ];
